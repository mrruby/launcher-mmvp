...
use crate::commands::save_app::{fetch_and_save_app, fetch_gui, save_app};
...
use crate::commands::{
    ...
    config::{get_default_bootstrap, get_default_signaling, write_config},
    enable_app::{delete_clone, disable_app, enable_app},
    ...
    notifications::{
        clear_happ_notifications, clear_systray_icon, notify_os, notify_tauri,
...
fn main() {
    let builder_result = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let profile: Profile = match cli_matches.args.get("profile") {
                Some(data) => match data.value.clone() {
                    Value::String(profile) => {
                        ...
                        {
                            eprintln!(
                                "Error: \"/\", \"\\\" and \"?\" are not allowed in profile names."
                            );
                            panic!(
                                "Error: \"/\", \"\\\" and \"?\" are not allowed in profile names."
                            );
                    ...
                    move |app, argv, cwd| {
                        ...
                        if let Some(window) = admin_window {
                            ...
                            let local_storage_path =
                                profile_tauri_dir(String::from("default")).unwrap();
                            let r =
                                build_admin_window(&app.app_handle(), local_storage_path).unwrap();
            ...
            let pubkey_map: Arc<Mutex<HashMap<String, AgentPubKey>>> =
            ...
            let launcher_state =
                tauri::async_runtime::block_on(
                    async move { launch_manager(handle, profile).await },
                );
...
async fn launch_manager(
    ...
    profile: Profile,
) -> RunningState<LauncherManager, LauncherError> {
    let holochain_dir = match profile_holochain_data_dir(profile.clone()) {
        Ok(dir) => dir,
        Err(e) => {
            log::error!(
                "Failed to get holochain data dir when trying to launch the LauncherManager: {}",
                e
            );
