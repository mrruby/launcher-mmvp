...
use crate::launcher::{manager::HolochainId, state::LauncherState};
...
pub async fn enable_app(
    ...
    if window.label() != "admin" {
        return Err(String::from(
            "Unauthorized: Attempted to call an unauthorized tauri command. (D)",
        ));
    }
...
pub async fn disable_app(
    ...
    if window.label() != "admin" {
        return Err(String::from(
            "Unauthorized: Attempted to call an unauthorized tauri command.",
        ));
    }
...
pub async fn delete_clone(
    ...
    log::info!(
        "Deleted cloned cell: app_id = {}, cell_id = {:?}",
        app_id,
        cell_id
    );
