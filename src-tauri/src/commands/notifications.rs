...
use tauri::{api::notification::Notification, Icon, Manager};
...
pub async fn notify_os(
    ...
    let happ_window_focused = match app_handle.get_window(derive_window_label(&app_id).as_str()) {
        Some(happ_window) => happ_window
            .is_focused()
            .map_err(|e| format!("Failed to get focus state of happ window: {}", e))?,
        None => false,
    };
    let admin_window_focused = window
        .is_focused()
    ...
    if !admin_window_focused && !happ_window_focused {
        for message in notifications {
            if systray {
                change_systray_icon_state(&app_handle, &message.urgency)
                    .await
...
pub async fn clear_happ_notifications(
    ...
    if let Some(admin_window) = app_handle.get_window("admin") {
        admin_window
            .emit("clear-happ-notifications", app_id)
...
#[tauri::command]
pub async fn clear_systray_icon(app_handle: tauri::AppHandle) -> tauri::Result<()> {
    let mutex = app_handle.state::<Mutex<SysTrayIconState>>();
    let icon_path_option = app_handle
        .path_resolver()
