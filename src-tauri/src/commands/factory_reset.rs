...
use crate::file_system::{
    holochain_version_data_dir, profile_config_dir, profile_holochain_data_dir, profile_lair_dir,
    profile_logs_dir, profile_tauri_dir, Profile,
};
...
pub async fn execute_factory_reset(
    ...
    if window.label() != "admin" {
        return Err(String::from(
            "Unauthorized: Attempted to call an unauthorized tauri command. (E)",
        ));
    }
    ...
    if delete_all_holochain_versions == true {
        ...
        remove_dir_if_exists(holochain_data_dir).map_err(|err| {
        ...
        .map_err(|e| {
          ...
          format!("Failed to get data directory of holochain version {:?} during factory reset: {:?}", version, e)
            ...
            remove_dir_if_exists(holochain_version_data_dir).map_err(|err| {
                log::error!(
                    "Could not remove data directory of holochain version {:?}: {:?}",
                    version,
                    err
                );
                format!(
    ...
    if cfg!(not(target_os = "windows")) {
        ...
        remove_dir_if_exists(tauri_data_dir).map_err(|err| {
