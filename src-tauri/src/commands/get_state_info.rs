...
use holochain_manager::versions::{version_manager::VersionManager, HolochainVersion};
...
use crate::{
    ...
    launcher::{
        ...
        state::{
            HolochainInfo, HolochainState, LauncherState, LauncherStateInfo,
...
pub async fn get_state_info(
    ...
    if window.label() != "admin" {
        return Err(LauncherError::Unauthorized(
            "Attempted to call an unauthorized tauri command. (G)".into(),
        ));
    }
...
> {
    ...
    match state {
        RunningState::Running(manager) => {
            ...
            let versions: Vec<HolochainVersion> =
                manager.holochain_managers.keys().cloned().collect();

            for holochain_version in versions {
                match manager
                    .get_web_happ_manager(HolochainId::HolochainVersion(holochain_version.clone()))
                {
...
async fn get_holochain_state(
    ...
    match web_app_manager.list_apps().await {
        Ok(installed_apps) => RunningState::Running(HolochainInfo {
            ...
            hdi_version: web_app_manager
                .holochain_manager
                .version
                .manager()
                .hdi_version(),
            hdk_version: web_app_manager
                .holochain_manager
                .version
                .manager()
                .hdk_version(),
        }),
        Err(err) => match retries {
