use crate::{file_system::Profile, launcher::state::LauncherState};
...
pub async fn initialize_keystore(
    ...
    if window.label() != "admin" {
        return Err(String::from(
            "Unauthorized: Attempted to call an unauthorized tauri command. (K)",
        ));
    }
...
pub async fn unlock_and_launch(
    ...
    manager
        .launch_managers(password, profile.inner().clone())
        .await?;
