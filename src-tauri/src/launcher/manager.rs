...
impl LauncherManager {
    ...
    pub async fn launch_managers(
        ...
        for version in holochain_versions_to_run {
            ...
            self.launch_holochain_manager(version, None, profile.clone())
                .await?;
        }

        if let Some(path) = self.config.custom_binary_path.clone() {
            self.launch_holochain_manager(HolochainVersion::custom(), Some(path), profile.clone())
                .await?;
        } else {
    ...
    pub async fn launch_holochain_manager(
        ...
        if let Some(path) = custom_binary_path.clone() {
            if !Path::new(&path).exists() {
                self.custom_binary_manager = Some(RunningState::Error(
                    LaunchWebAppManagerError::LaunchHolochainError(
        ...
        let environment_path = match custom_binary_path.is_some() {
            true => profile_holochain_data_dir(profile.clone())
                ...
                .join("custom"),
            false => holochain_version_data_dir(&version, profile.clone()).map_err(|e| {
                format!("Failed to get profile's holochain version data dir: {}", e)
            })?,
        };
        ...
        match holochain_id {
            HolochainId::HolochainVersion(version) => {
                if let None = self.holochain_managers.get(&version) {
                    self.launch_holochain_manager(version.clone(), None, profile)
                        .await?;
                }
            }
            HolochainId::CustomBinary => {
                ...
                if let None = self.custom_binary_manager {
                    self.launch_holochain_manager(HolochainVersion::custom(), Some(path), profile)
                        .await?;
                }
            }
        }
    ...
    pub fn get_web_happ_manager(
        ...
        let manager_state = match holochain_id {
            HolochainId::HolochainVersion(version) => {
                self.holochain_managers.get_mut(&version).ok_or(format!(
                    "Holochain version {} is not running.",
                    version.to_string()
                ))
            }
    ...
    pub fn open_app(
        ...
        window_builder = window_builder.initialization_script(include_str!(
            "../../../api_scripts/dist/launcher-api-scripts.js"
        ));
