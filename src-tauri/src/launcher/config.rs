...
use crate::{
    file_system::{launcher_config_path, Profile},
    BootstrapServerUrl, SignalingServerUrl,
};
...
impl LauncherConfig {
    ...
    pub fn read(profile: Profile) -> LauncherConfig {
        ...
        match fs::read_to_string(config_path) {
            Ok(str) => match serde_yaml::from_str::<LauncherConfig>(str.as_str()) {
                Ok(config) => config,
                Err(e) => {
    ...
    pub fn write(&self) -> Result<(), LauncherError> {
        let serde_config =
            serde_yaml::to_string(&self).expect("Could not serialize launcher config");
